...
**Status:** 🟡 Partial — Day 1+2 shipped (code + tests); Console listing-fields sync complete (all 3 fields fully populated, confirmed via `flex-cli search`); Day 1 inventory backfill complete for all 25 priority (`.env.dev`-live) brands, 9 non-priority brands deferred to a separate task; Day 3 not started
...
- [x] Backfill run; `unmapped-item-aspect-options.json` shows no new occasion/gift/recipient leakage — see 2026-09-05 update.
...
**Update (2026-09-05):** Inventory backfill complete for all 25 priority brands (the brands with a real Sharetribe user ID in `product-listing-integration/.env.dev` — i.e. actually live, not placeholder `ADD`/`add`), run prod-tier (`scraped_products_prod/` → `classified_products_prod/`, not dev — that's what `product-listing-integration` reads to push live listings). Full commit list and per-brand row/field counts are in `Mela-scrapper-integrations` git history (`git log --grep="data(backfill): enrich-only"`).

- **Bestseller-only rule (new, applies going forward):** brands over **500** listings get `--enrich-only` run against an `isBestseller=True`-filtered temp CSV instead of the full catalog (cheaper, and the full catalog isn't needed for a gifting-traffic wedge). Affected this pass: isharya, nicobar, thenesavu_girls, thenesavu_baby, pluchi_home, superbottoms, pluchi, houseofchikankari, tarinika, thenesavu_boys. One exception — `nicobar_home` has no `bestseller_collection` configured in `shopify_brands.py` for its `homeware` collection entry, so `isBestseller` is `False` for all rows; it was run full-catalog instead. Fix at the source (add a `bestseller_collection` for that entry) before the next re-run, or the full-catalog fallback will keep applying.
- **Data-loss guard added:** each brand's re-enrichment is diffed against the prior git-committed `Item_Aspects` before committing — a brand is only committed if row count didn't shrink and no previously-non-empty `Item_Aspects` went blank. This caught real (if rare) cases — `gullylabs` (1 SKU), `baby_forest` (2 SKUs), `fizzygoblet` (3 SKUs), `thealternate` (1 SKU) — where the model returned an empty `Item_Aspects` for a low-signal product (a "freebie" add-on, a simple sock) on re-enrichment while everything else in that call succeeded (fresh `SEO_Title`/`Meta_Description`/category). Not the batch-failure bug (already fixed pre-backfill) — a separate, occasional per-product miss. Resolved by restoring just the affected SKUs' prior `Item_Aspects` and keeping the rest of that run's improvements, rather than discarding the whole brand.
- **`unmapped-item-aspect-options.json` cross-check:** `gift_occasion` and `recipient` — **0 unmapped values** across the entire backlog; every LLM-generated value validated cleanly against `configListing.js`'s enum. `occasion` — 69 pre-existing unmapped values remain (e.g. `daily wear`, `casual_wear`, `school functions`), spanning The Nesavu, House of Chikankari, Isharya, Tarinika, and others — confirmed via `git show HEAD~1` that these values predate this backfill (pulled verbatim from raw scraped descriptions, not something `--enrich-only` introduced or worsened). This is a pre-existing `occasion`-field prompt-adherence gap, out of scope for this PRD's acceptance bar (which is about *new* leakage from `gift_occasion`/`recipient`) but worth a follow-up to tighten `prompt_engine.py`'s bounded-enum instruction for `occasion`.
- **9 non-priority brands** (not live in `.env.dev`: malabarbaby, myfirstcrayons, saphed_kids, skillmatics, tibertaber, comet, pastelsandpop, saphed, chidiyaa) are **not yet backfilled** — deferred to a separate task per user instruction.
- Still open from acceptance above: the 10-sample-product spot-check and the Sharetribe Console/search verification (`pub_gift_occasion=has_any:diwali`) — `product-listing-integration` needs to actually push these classified CSVs to Sharetribe first (see the `cli-utils.js` `classified_products_prod` fix already applied, separately, in that repo).
...
**Update (2026-08-27):**
- `gift_occasion` field created in Console (2 placeholder options so far; full 18-option list at `configListing.js:601-620` for later). Its search schema auto-registered on creation — `flex-cli search -m mela-dev` now shows it as `multi-enum`. `recipient` field creation not yet confirmed the same way (not showing in the schema list) — verify it saved with Public scope.
- **Third gap found + fixed**: confirmed with Sharetribe's Developer Advocate team that `has_any`/`has_all` matching is an API-call-time choice, not schema-level — but Console has no UI to set it, so any Console-managed multi-enum field validates to `has_all` (AND) by default via `configHelpers.js`'s `validSearchMode`. Without a fix, the *generic* SearchPage filter panel (not GiftingPage's own hand-built `has_any:` links, which were already unaffected) would have built AND-semantics queries for `occasion`/`gift_occasion`/`recipient` — a shopper selecting multiple checkboxes would get listings matching ALL selected values instead of ANY. Fixed in `configHelpers.js` (`applyHasAnySearchModeOverride`, commit `87c3832a2`) with a hardcoded key-based override, per Sharetribe's suggested approach. Tested (`configHelpers.applyHasAnySearchModeOverride.test.js`, 6 tests); 154/154 suites green repo-wide.
- Still open: the 18 new `occasion` enum values in Console (first gap above), and confirming `recipient`'s Console setup.
