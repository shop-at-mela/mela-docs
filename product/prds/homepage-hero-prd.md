...
5. ~~**Isharya vs. Tarinika for the first-fold jewelry slot**~~ — **RESOLVED (2026-07-15): Tarinika takes the slot.** Isharya is global-editorial statement jewelry (LA HQ), which mismatches the §12A "kundan/gold close-up" rationale; Tarinika's temple-inspired design gives the stronger traditional India-shout the fold rule calls for. Isharya demoted to the breadth-builder tier. `configBrands.js` `CURATED_BRAND_SLUG_ORDER` updated to match. *(Original open framing, kept for context: Isharya is global-editorial statement jewelry (LA HQ), which mismatches the §12A "kundan/gold close-up" rationale. Either re-justify Isharya on "bold modern Indian jewellery," or swap to Tarinika (temple-inspired → stronger traditional India-shout). Decide once one of them ingests with live product photography.)*
...
2. **Step 4 — Generate static hero imagery** in Blotato from Fizzy Goblet + Nicobar listings (per IG visual guide).
   - **Lifestyle hero photo — sourcing strategy (2026-07-30, from homepage-redesign mockup pt 3), ranked for 0→1:** (1) **reuse the existing Blotato natural-scene pipeline** already used for social — real product images composited into earthy, lifestyle-staged scenes (no white backgrounds, no text overlays, per the IG visual guide) — cheapest path to 1–2 hero composites; (2) **brand-supplied lifestyle imagery** — many partners already have editorial shots we can request; (3) commissioned / UGC later. **Avoid generic stock** (breaks the curated-boutique feel). Keep the hero slot a labeled placeholder in mocks until a real asset is chosen. Note Appendix A of `homepage-redesign-prd.md` set aside *AI-composited single-brand* hero scenes as inauthentic to a craft-fluent audience — this is lifestyle staging of real products, not a synthetic multi-category collage.
