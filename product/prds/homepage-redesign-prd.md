...
- **Diaspora-aware search bar** *(2026-08-17 — from the search-engineering + social panel; ties to `search-ranking-relevance-prd.md` §4B)*: the homepage search input advertises Mela's discovery moat at first touch. Rotating placeholder / autosuggest seeded with the **bilingual vocabulary** the search index now carries — `Try "kurta", "jhula", "jhumka"…`. This signals "this search speaks your language" the moment a diaspora visitor lands, and directly demonstrates the differentiator generic marketplaces lack. Cheap: placeholder copy + the synonym terms already generated by the ingestion pipeline. **Dependency:** the `searchKeywords`/`search_synonyms` `text` fields must be registered + indexed (search PRD §4D.1) for the terms to actually resolve; until then the placeholders still work as signaling but should use terms known to return results. Copy must anchor *situation, not identity* — placeholder shows garment/product words, never "search in Hindi" or an ethnic label (see UXR cultural-framing principle + `visual-style-guide.md` → Diaspora Anchoring).

### Nice to Have (P2)
- **Curated discovery rows from the Mela Relevance Score** *(2026-08-17 — depends on `search-ranking-relevance-prd.md` Phase A shipping `melaScore`/`boostTier`)*: 1–3 editorial rows above the fold sourced from the same curation signal the SRP uses — **"Mela's Picks"** (top `melaScore`), **"Certified & Safe"** (values facet: `certification` + `sustainability_claim`), **"New Brands to Know"** (the discovery/new-brand boost). Keeps the homepage inspiration-first: editorial framing, *not* "Trending now / algorithmic." Cap at 2–3 rows so the page still reads curated, not exhaustive. **Hard dependency:** `melaScore` must exist and be backfilled first — do not build against recency.
- **Trending / real-searches module** *(depends on query-log instrumentation, search PRD §7)*: a small "What families are searching" module driven by real query logs — social proof + discovery. Deferred until search analytics land.
...
- No backend changes required for the core P0/P1 reframe — all changes are frontend copy and component additions
- **Diaspora search bar + curated melaScore rows (P1/P2) depend on `search-ranking-relevance-prd.md`**: the search bar needs `searchKeywords`/`search_synonyms` `text` schemas registered + indexed (that PRD §4D.1); the curated rows need `melaScore`/`boostTier` computed + backfilled (that PRD Phase A). These are *not* frontend-only — sequence them after the search pipeline work, or ship the search bar with placeholder terms known to already return results.
