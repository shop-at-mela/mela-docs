...
- [ ] Confirm `REACT_APP_CSP=block` doesn't silently break anything that only showed up as a report-only violation before (GTM, GA4, Clarity, Stripe, Mapbox all need to be in the CSP allowlist already — per `server/csp.js`, GTM/GA4/Clarity domains are confirmed present from the crossshop-tracking work; re-verify Stripe/Mapbox aren't newly blocked once mode flips to `block`).
