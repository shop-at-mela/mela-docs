...
- [ ] **`configBrands.js`'s per-brand `bestsellerProductIds` pools are Dev-only** (a rate-limit optimization — `BrandsPage.duck.js` batch-fetches bestsellers by these ids on Dev to avoid the per-brand `author_id` fan-out that trips Sharetribe's ~1 req/sec Dev limit). **No production action required**: brands without a pool fall back automatically to the live `author_id + pub_isBestseller` query, so an empty production config behaves correctly. Optionally re-harvest pools against the production environment (`web-client/scripts/harvest-bestseller-ids.js`) if/when production ever hits the same rate ceiling.
