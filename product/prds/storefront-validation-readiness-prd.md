...
- [x] **REWORK (decision 2026-07-26), done same day, then reworked again same day**: outbound store link/CTA removed from the hero band — a prior pass had implemented it there and checked this AC off; it was then undone and relocated to a plain "Brand website" link in the About & Story tab. That link has now **also** been removed entirely (same-day follow-up decision): an above-the-fold or below-the-fold exit door with no affiliate tracking to show for it is a pure loss either way. `brandStoreUrl` is no longer rendered anywhere on `/brands/:brandSlug` or in the homepage Brand Spotlight module — `RedirectTrustSheet`/`openBrandStorefront` are no longer wired to either page. Hero band's sole CTA is the on-Mela "Browse {N} Products" anchor (BrandStorefront) / "See {Brand} on Mela" link (BrandSpotlight). Product-level redirect flow on individual listing pages is unchanged and is now the **only** outbound path anywhere in the app. Test suites updated and passing (BrandStorefront 27/29, BrandSpotlight 5/5; full suite 140/140, 2263 tests).
