...
| [gifting-festival-traffic-prd.md](prds/gifting-festival-traffic-prd.md) | 🟡 Partial | P0 | D1+D2 code shipped; Console listing-fields asset + `flex-cli search set` (gift_occasion/recipient) still needed before new occasion values display correctly; D3 not started | D3: social calendar/boards + brand sourcing + ads-readiness gate; Console sync for new occasion/gift_occasion/recipient enum values; `flex-cli search set` for gift_occasion + recipient |
