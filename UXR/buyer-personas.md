# Mela Buyer Personas

Detailed buyer personas for UX research and development testing.

---

## Persona 1: Sarah Chen - The Conscious American Parent

```
┌─────────────────────────────────────────────────────────────────┐
│ SARAH CHEN                                                      │
│ "I want real sustainability, not marketing buzzwords"           │
├─────────────────────────────────────────────────────────────────┤
│ DEMOGRAPHICS                                                    │
│ • 34 years old                                                  │
│ • Lives in Portland, OR                                         │
│ • Marketing Manager at a tech company                           │
│ • Household income: $145K                                       │
│ • Married, one child (18 months), expecting second              │
├─────────────────────────────────────────────────────────────────┤
│ BACKGROUND                                                      │
│ • Shops at Whole Foods, farmers markets                         │
│ • Follows sustainability influencers on Instagram               │
│ • Has been burned by "greenwashing" brands before               │
│ • Willing to pay 2-3x for genuinely sustainable products        │
│ • Does extensive research before purchasing                     │
│ • Member of local "conscious parenting" Facebook group          │
├─────────────────────────────────────────────────────────────────┤
│ SHOPPING BEHAVIOR                                               │
│ • Researches for days before buying baby products               │
│ • Reads reviews on multiple sites                               │
│ • Checks certifications (knows GOTS, OEKO-TEX, B Corp)          │
│ • Looks up brand founders and company stories                   │
│ • Prefers fewer, higher-quality items over many cheap ones      │
│ • Shares finds with her parenting community                     │
├─────────────────────────────────────────────────────────────────┤
│ GOALS                                                           │
│ • Find baby products that are ACTUALLY sustainable              │
│ • Avoid mass-produced fast fashion for her kids                 │
│ • Support artisans and fair trade when possible                 │
│ • Discover unique products not available at Target/Amazon       │
│ • Feel good about her purchasing decisions                      │
├─────────────────────────────────────────────────────────────────┤
│ PAIN POINTS                                                     │
│ • "Every brand claims to be sustainable now"                    │
│ • "I can't tell what's real vs marketing"                       │
│ • "Etsy is hit or miss - quality varies wildly"                 │
│ • "I want to support small makers but discovery is hard"        │
│ • "International shipping usually means long waits and high $"  │
├─────────────────────────────────────────────────────────────────┤
│ TRUST SIGNALS                                                   │
│ ✓ Third-party certifications (GOTS, OEKO-TEX, Fair Trade)      │
│ ✓ Transparent supply chain / "made by" stories                 │
│ ✓ Real artisan photos (not stock images)                       │
│ ✓ Detailed material information                                │
│ ✓ Customer reviews mentioning quality                          │
│ ✗ Vague sustainability claims without proof                    │
│ ✗ "Eco-friendly" without specifics                             │
│ ✗ No information about who made the product                    │
├─────────────────────────────────────────────────────────────────┤
│ TECH COMFORT: ★★★★★ High                                        │
│ PRICE SENSITIVITY: ★★☆☆☆ Low (pays premium for quality)        │
│ PATIENCE: ★★★☆☆ Medium (will wait for right product)           │
└─────────────────────────────────────────────────────────────────┘
```

### Sarah's User Journey

1. **Discovery**: Searching "organic baby clothes not Amazon" or sees Instagram ad
2. **Evaluation**: Spends 10+ minutes reading brand story, checking certifications
3. **Comparison**: Opens multiple tabs, compares with other sustainable brands
4. **Decision**: Certification + artisan story + reviews = purchase
5. **Post-purchase**: Shares on social media if impressed, joins community

### Key Questions Sarah Asks

- "Who actually made this?"
- "What certifications do they have?"
- "Is this a real small brand or just marketing?"
- "What are the materials, specifically?"
- "Do other conscious parents recommend this?"

### Sarah's Relationship With "Indian" in Copy
Sarah is not Indian and does not identify with that label. She is shopping on Mela because of craft quality and genuine sustainability — not to collect "ethnic goods."
- **"Indian" on products** (e.g. "Made in Jaipur by artisans"): ✅ Signals authenticity and supply-chain transparency — exactly what she wants
- **"Indian" applied to her** (e.g. "your favourite Indian finds"): ❌ Reframes her as a cultural tourist, triggers subtle guilt about cultural appropriation — she never feels this on Etsy
- **Key copy principle**: Let the product description carry the cultural origin; let her personal layer be about her taste, not her ethnicity

---

## Persona 2: Priya Mehta-Johnson - The Mixed Heritage Parent

```
┌─────────────────────────────────────────────────────────────────┐
│ PRIYA MEHTA-JOHNSON                                             │
│ "I want my kids to know their Indian heritage"                  │
├─────────────────────────────────────────────────────────────────┤
│ DEMOGRAPHICS                                                    │
│ • 31 years old                                                  │
│ • Lives in Chicago suburbs                                      │
│ • Nurse practitioner                                            │
│ • Household income: $180K                                       │
│ • Married to Matt (white American), two kids (3 yrs, 6 months)  │
├─────────────────────────────────────────────────────────────────┤
│ BACKGROUND                                                      │
│ • Born in US, parents immigrated from Gujarat                   │
│ • Celebrates both American and Indian holidays                  │
│ • Wants kids to feel connected to Indian culture                │
│ • Matt is enthusiastic about learning Indian traditions         │
│ • Parents (kids' grandparents) live nearby, very involved       │
│ • Shops mostly at Target, Old Navy for everyday items           │
├─────────────────────────────────────────────────────────────────┤
│ SHOPPING BEHAVIOR                                               │
│ • Shops for Indian items around Diwali, Holi, birthdays         │
│ • Has bought from random Etsy sellers with mixed results        │
│ • Mom sometimes brings clothes from India trips                 │
│ • Doesn't know Indian baby brands (left India as infant)        │
│ • Matt often initiates searches for "Indian baby outfits"       │
│ • Willing to spend more for special occasion / cultural items   │
├─────────────────────────────────────────────────────────────────┤
│ GOALS                                                           │
│ • Find authentic Indian clothes for festivals and photos        │
│ • Products that help explain culture to her kids                │
│ • Gift-worthy items to share with non-Indian friends            │
│ • Build collection of meaningful cultural items                 │
│ • Make her parents happy (they love seeing grandkids in Indian) │
├─────────────────────────────────────────────────────────────────┤
│ PAIN POINTS                                                     │
│ • "I can't tell authentic from 'Indian-inspired' costume"       │
│ • "Amazon 'ethnic' clothes look cheap and inaccurate"           │
│ • "I don't know which Indian brands are actually good"          │
│ • "Sizing from India is confusing"                              │
│ • "I want my kids to wear it proudly, not look like a costume"  │
├─────────────────────────────────────────────────────────────────┤
│ TRUST SIGNALS                                                   │
│ ✓ "Made in India" with specific region/artisan info            │
│ ✓ Cultural context (what occasion is this for?)                │
│ ✓ Modern aesthetic (not costume-y)                             │
│ ✓ Size guide with US comparisons                               │
│ ✓ Photos of real kids wearing the clothes                      │
│ ✗ Generic "ethnic" or "Bollywood" descriptions                 │
│ ✗ Halloween-costume aesthetic                                  │
│ ✗ No cultural context or explanation                           │
├─────────────────────────────────────────────────────────────────┤
│ TECH COMFORT: ★★★★☆ High                                        │
│ PRICE SENSITIVITY: ★★★☆☆ Medium (splurges for special items)   │
│ PATIENCE: ★★★★☆ High (plans ahead for occasions)               │
└─────────────────────────────────────────────────────────────────┘
```

### Priya's User Journey

1. **Discovery**: Searching "Indian baby outfit Diwali" or recommendation from Indian mom group
2. **Evaluation**: Looks for authenticity cues, modern aesthetic, cultural explanation
3. **Sharing**: Shows Matt - they decide together
4. **Decision**: If it looks authentic + appropriate for occasion + good reviews
5. **Post-purchase**: Photos shared with parents, posted on social media during festival

### Key Questions Priya Asks

- "Is this actually from India or just 'Indian-style'?"
- "What occasion is this appropriate for?"
- "Will this fit my American-sized kid?"
- "Does this look dignified or costume-y?"
- "Would my mom approve of the quality?"

### Priya's Relationship With "Indian" in Copy
Priya actively wants cultural context at the *product* level — she needs to know if a lehenga is Bandhani, when a garment is appropriate for Diwali vs. a casual playdate. But her *personal* collection isn't her ethnicity — it's her life as both an American and someone reclaiming heritage.
- **"Indian" on products**: ✅ Essential — she needs to know provenance and cultural occasion
- **"Indian" applied to her saves/collection**: ❌ Creates an invisible wall between "her Indian stuff" and "the family's stuff" when browsing with her husband Matt or non-Indian friends. Her children are mixed-heritage — "Indian finds" erases that
- **Key copy principle**: Frame cultural context around occasions and product stories, not the user's identity

---

## Persona 3: Neha Krishnamurthy - The First-Gen Immigrant

```
┌─────────────────────────────────────────────────────────────────┐
│ NEHA KRISHNAMURTHY                                              │
│ "I know what's good, I just need a way to get it here"          │
├─────────────────────────────────────────────────────────────────┤
│ DEMOGRAPHICS                                                    │
│ • 33 years old                                                  │
│ • Lives in Bay Area (Sunnyvale)                                 │
│ • Software engineer at Google                                   │
│ • Household income: $350K                                       │
│ • Married to Karthik (also from India), one child (8 months)    │
├─────────────────────────────────────────────────────────────────┤
│ BACKGROUND                                                      │
│ • Moved to US 6 years ago for master's degree                   │
│ • Grew up in Chennai, knows Indian brands well                  │
│ • Parents and in-laws visit 1-2 times per year                  │
│ • Currently asks family to bring baby clothes when visiting     │
│ • Active in Bay Area Indian mom WhatsApp groups                 │
│ • Shops on Myntra/FirstCry when in India                        │
├─────────────────────────────────────────────────────────────────┤
│ SHOPPING BEHAVIOR                                               │
│ • On Myntra/FirstCry: demand-fulfillment mode — knows brands    │
│ • On Mela: switches to inspiration mode — open to brands she    │
│   didn't know to look for yet                                   │
│ • Frustrated that preferred brands don't ship to US easily      │
│ • Compares US prices to India prices (expects some markup)      │
│ • Bulk orders when family visits India                          │
│ • Recommends brands to other Indian moms in her network         │
│ • Price-conscious but values convenience highly                 │
├─────────────────────────────────────────────────────────────────┤
│ GOALS                                                           │
│ • Buy familiar Indian brands without waiting for family visits  │
│ • Get good prices (not 3x India prices)                         │
│ • Reliable shipping with tracking                               │
│ • Same quality she'd get in India                               │
│ • Organic/safe products she trusts from Indian market           │
├─────────────────────────────────────────────────────────────────┤
│ PAIN POINTS                                                     │
│ • "Direct shipping from India costs ₹2000+ per order"           │
│ • "I have to plan 2 months ahead for family to bring items"     │
│ • "Some brands I love don't ship internationally at all"        │
│ • "US sellers charge 2-3x India prices - I know what it costs!" │
│ • "Brands think we're rich in US so they jack up prices"        │
│ • "I want to just browse and buy like I would on Myntra"        │
├─────────────────────────────────────────────────────────────────┤
│ PRICE SENSITIVITY (KEY INSIGHT)                                 │
│ Neha is highly price-aware because she KNOWS India retail:      │
│ • Checks Myntra/FirstCry prices before buying in US             │
│ • Acceptable markup: 30-50% over India (covers shipping)        │
│ • Unacceptable: 2-3x markup ("they think we're stupid")         │
│ • Will wait for family visit if US price is too high            │
│ • Shares price comparisons in WhatsApp groups                   │
├─────────────────────────────────────────────────────────────────┤
│ TRUST SIGNALS                                                   │
│ ✓ Brands she already knows and trusts                          │
│ ✓ Fair markup over India prices (30-50% acceptable)            │
│ ✓ Clear shipping timeline and tracking                         │
│ ✓ Same product photos as official brand site                   │
│ ✓ Reviews from other Indian moms in US                         │
│ ✗ Unknown brands with no India presence                        │
│ ✗ 2-3x price markup (instant dealbreaker)                      │
│ ✗ Vague shipping estimates ("2-4 weeks")                       │
├─────────────────────────────────────────────────────────────────┤
│ TECH COMFORT: ★★★★★ Very High                                   │
│ PRICE SENSITIVITY: ★★★★★ Very High (knows India prices exactly)│
│ PATIENCE: ★★☆☆☆ Low (wants convenience)                        │
└─────────────────────────────────────────────────────────────────┘
```

### Neha's User Journey

1. **Discovery**: Word of mouth in Indian mom groups, or searching for specific brand
2. **Evaluation**: Checks if her preferred brands are available, compares prices
3. **Validation**: Asks in WhatsApp group if others have ordered
4. **Decision**: If brand is known + price is fair + shipping is clear
5. **Post-purchase**: Recommends to her network if experience is good

### Key Questions Neha Asks

- "Do they have [specific brand] I know from India?"
- "What's the total cost including shipping?"
- "How much is this on Myntra/FirstCry?" (she WILL check)
- "Why is this ₹800 product selling for $40 here?"
- "How long will it actually take to arrive?"
- "Is this the same quality as buying in India?"
- "Have other people in my community ordered from here?"

### Neha's Shopping Mode on Mela

Neha is the most behaviorally split persona — she has two distinct modes that don't always coincide with a single session:

- **Fulfillment mode** (what she does on Myntra): Arrives knowing the brand, the SKU, often the approximate price. Any platform that adds friction to this path loses her immediately.
- **Inspiration mode** (what she does on Mela): Arrives with a loose intent ("something for the baby," "a festival outfit") and is genuinely open to being surprised. This is where Mela can expand her brand repertoire beyond what she already knows from India.

**The sub-variant to know:** First-gen immigrants who came to the US *via marriage* (rather than for education or work) arrive at Mela more strongly in inspiration mode. They may shop Indian brands, but they built their brand familiarity differently — through a partner's family, through home visits, through WhatsApp group sharing — rather than through years of independently browsing Myntra. These users have strong cultural taste but less rigid brand loyalty, which makes curation particularly valuable to them.

**Design implication:** Always surface familiar brands fast (fulfillment path), but invest equally in the discovery layer — curated collections, "you might also like from Indian artisans," occasion-based browsing. Neha in inspiration mode is where Mela earns loyalty beyond convenience.

### Neha's Relationship With "Indian" in Copy
Neha grew up in Chennai with a strong Tamil identity. "Indian" as a monolith is both accurate and reductive for her — it is true but it collapses Carnatic music, Kanchipuram silk, and Pongal traditions into the same bucket as Punjabi bhangra wear and Rajasthani block print.
- **"Indian" on products**: ✅ Acceptable at the macro level; she prefers more specific regional/brand attribution when possible ("Fabindia" or "Tirupur cotton" land better than "Indian cotton")
- **"Indian" applied to her collection**: ❌ Redundant and slightly reductive — she knows what she saved and why; the label adds nothing
- **Key copy principle**: Regional specificity beats national generality for trust. "Indian" is a floor, not a ceiling

---

## Persona 4: Arun Patel - The Second-Gen Cultural Reclaimer

```
┌─────────────────────────────────────────────────────────────────┐
│ ARUN PATEL                                                      │
│ "I want my daughter to have what I didn't growing up"           │
├─────────────────────────────────────────────────────────────────┤
│ DEMOGRAPHICS                                                    │
│ • 36 years old                                                  │
│ • Lives in Austin, TX                                           │
│ • Product Manager at a startup                                  │
│ • Household income: $200K                                       │
│ • Married to Anjali (also second-gen), one daughter (2 years)   │
├─────────────────────────────────────────────────────────────────┤
│ BACKGROUND                                                      │
│ • Born in New Jersey, grew up feeling "between cultures"        │
│ • Parents are from Gujarat but didn't emphasize culture much    │
│ • Reconnecting with heritage after becoming a parent            │
│ • Learning Hindi/Gujarati as an adult                           │
│ • Wants daughter to have strong cultural identity he lacked     │
│ • Follows Indian-American parenting accounts on Instagram       │
├─────────────────────────────────────────────────────────────────┤
│ SHOPPING BEHAVIOR                                               │
│ • Doesn't know Indian brands (has to Google everything)         │
│ • Attracted to "modern Indian" aesthetic                        │
│ • Wants products that feel cultural but not "too traditional"   │
│ • Often buys matching parent-child outfits for festivals        │
│ • Wife Anjali usually makes final purchase decision             │
│ • Values aesthetics and Instagram-worthiness                    │
├─────────────────────────────────────────────────────────────────┤
│ GOALS                                                           │
│ • Build cultural connection through everyday objects            │
│ • Products that spark conversations about heritage              │
│ • Modern aesthetic his American friends would appreciate        │
│ • Feel confident he's doing "Indian parenting" right            │
│ • Create the cultural childhood he wishes he had                │
├─────────────────────────────────────────────────────────────────┤
│ PAIN POINTS                                                     │
│ • "I don't know what's authentic vs. stereotypical"             │
│ • "Some Indian stuff feels too intense for our lifestyle"       │
│ • "I feel like I'm not Indian enough to buy this"               │
│ • "I want guidance, not assumptions I know things"              │
│ • "Will other Indians judge my choices as inauthentic?"         │
├─────────────────────────────────────────────────────────────────┤
│ TRUST SIGNALS                                                   │
│ ✓ Modern, elevated aesthetic                                   │
│ ✓ Cultural context explained (don't assume knowledge)          │
│ ✓ Products featured by Indian-American influencers             │
│ ✓ "Entry points" for cultural beginners                        │
│ ✓ Mix of everyday + special occasion items                     │
│ ✗ Too traditional or overwhelming                              │
│ ✗ Assumes prior cultural knowledge                             │
│ ✗ Feels like cultural gatekeeping                              │
├─────────────────────────────────────────────────────────────────┤
│ TECH COMFORT: ★★★★★ Very High                                   │
│ PRICE SENSITIVITY: ★★☆☆☆ Low (invests in meaningful items)     │
│ PATIENCE: ★★★☆☆ Medium (wants things for specific occasions)   │
└─────────────────────────────────────────────────────────────────┘
```

### Arun's User Journey

1. **Discovery**: Instagram ad, or searching "Indian baby clothes modern"
2. **Evaluation**: Looking for modern aesthetic, not "too traditional"
3. **Validation**: Shows Anjali, checks if influencers have featured it
4. **Decision**: If it feels accessible, modern, and meaningful
5. **Post-purchase**: Posts on Instagram, shares with other second-gen parents

### Key Questions Arun Asks

- "Will this help my daughter connect with her heritage?"
- "Is this modern enough for our aesthetic?"
- "Do I need to know something cultural to use this?"
- "Will other Indian people think this is authentic?"
- "Can I wear/use this without feeling like I'm playing dress-up?"

### Arun's Relationship With "Indian" in Copy
This is the most psychologically sensitive case. Arun experiences low-grade impostor syndrome about his Indian identity — he's reconnecting as an adult and is uncertain whether he is "Indian enough" to make certain purchases. UI copy that repeatedly labels his actions as "Indian" applies subtle pressure.
- **"Indian" on products**: ✅ Discovery tool for him — he doesn't know Indian brands, so "Indian design" as a category label helps him find things
- **"Indian" applied to his collection**: ❌ Quietly reinforces the question "Am I Indian enough to save these?" His saves should feel like expressions of *his* evolving taste — not a scored performance of Indian-ness. `"Your favourite Mela finds"` respects his agency; `"your favourite Indian finds"` categorizes him
- **Key copy principle**: Describe the cultural origin of products; never describe the user by their cultural origin

---

## Persona Comparison Matrix

| Attribute | Sarah | Priya | Neha | Arun |
|-----------|-------|-------|------|------|
| **Primary driver** | Sustainability | Heritage for kids | Convenience + fair price | Cultural identity |
| **Brand knowledge** | None | Low | High | None |
| **Price sensitivity** | Low | Medium | Very High (knows India prices) | Low |
| **Cultural knowledge** | None | Medium | High | Low |
| **Trust priority** | Certifications | Authenticity | Brand + price fairness | Accessibility |
| **Shopping trigger** | Everyday need | Festivals | Ongoing need | Occasions + identity |
| **Key fear** | Greenwashing | Costume-y look | Being overcharged (knows real cost) | Not being "Indian enough" |
| **Shopping mode on Mela** | Pure inspiration | Occasion-led inspiration | Split (fulfillment + inspiration) | Pure inspiration |
| **"Indian" on products** | ✅ Trust signal | ✅ Essential for authenticity | ✅ Acceptable; regional is better | ✅ Discovery tool |
| **"Indian" on user layer** | ❌ Cultural tourism framing | ❌ Erases her American identity | ❌ Reductive monolith | ❌ Amplifies impostor syndrome |

---

## Using Personas in Development

### Feature Testing Matrix

| Feature | Sarah Focus | Priya Focus | Neha Focus | Arun Focus |
|---------|-------------|-------------|------------|------------|
| Product page | Certifications visible? | Cultural context? | Brand recognition? | Modern aesthetic? |
| Category browse | Filter by certification? | Filter by occasion? | Filter by brand? | Entry-level options? |
| Checkout | Shipping sustainability? | Gift wrapping? | Total cost clarity? | Smooth flow? |
| Brand page | Artisan story depth? | Heritage explanation? | India presence? | Accessibility? |
| Search | "Organic" works? | "Diwali" works? | Brand name works? | "Modern Indian" works? |

### Red Flags by Persona

**Sarah sees red flag if:**
- No certifications mentioned
- Vague "eco-friendly" claims
- Stock photos instead of real artisan images
- No material details

**Priya sees red flag if:**
- Looks like Halloween costume
- No cultural context ("what is this for?")
- "Bollywood" or generic "ethnic" language
- No size guide for US comparison

**Neha sees red flag if:**
- Brands she doesn't recognize
- Price > 2x India retail (she will check Myntra/FirstCry)
- No price justification for markup (shipping, duties?)
- Unclear shipping timeline
- Different photos than brand's official site
- Feels like "US pricing" exploitation

**Arun sees red flag if:**
- Feels inaccessible or "too traditional"
- Assumes he knows Hindi/customs
- No modern styling options
- Feels like cultural gatekeeping
- UI copy labels his actions as "Indian" (amplifies impostor syndrome)

---

## Cross-Cutting UXR Insight: The "Indian" Anchoring Principle

> *Updated: April 2026 — based on copy review of saved-items feature and deep persona analysis*

### The Three Layers — Only Two Are Healthy

There are three places "Indian" can appear in Mela's product experience. The first two create trust. The third causes harm across all four personas.

| Layer | Example | Effect |
|-------|---------|--------|
| **Product identity** | "Handwoven in Varanasi" / "GOTS certified, made in India" | ✅ Accurate, trust-building, always use |
| **Brand identity** | "Mela" (Hindi: gathering/fair) / "100+ Indian brands" | ✅ Brand carries cultural meaning so copy doesn't have to |
| **User identity** | "your favourite Indian finds" / "for Indian parents" | ❌ Reduces user to their ethnicity — harmful for all 4 personas differently |

### Why This Matters for Each Persona

- **Sarah**: "Indian finds" reframes her as a cultural tourist buying ethnic goods. She shops for quality and craft; the label implies her taste is about identity, not values.
- **Priya**: Her children are mixed-heritage. "Indian finds" creates an invisible wall when browsing with Matt or sharing items with non-Indian friends. Her cultural choices are part of her whole family life, not a separate ethnic bucket.
- **Neha**: "Indian" as a monolith flattens her Tamil/South Indian identity. She's not saving generic "Indian things" — she's saving specific brands she trusts from specific regions.
- **Arun**: He experiences impostor syndrome about not being "Indian enough." UI copy that labels his collection as "Indian finds" quietly scores his performance of Indian-ness on every hover interaction.

### The Rule

**"Mela" carries the cultural identity so copy doesn't have to.**

Every user who reaches the platform has already passed through the cultural frame — the brand name, the product catalog, the artisan stories all signal Indian design. The user's personal experience layer (saved items, collection, personal copy) should be about *their taste*, not their ethnicity.

| Copy context | Use "Indian"? | Use instead |
|-------------|--------------|------------|
| Product origin description | ✅ Always | "Handwoven in Rajasthan" / "Made by artisans in Tirupur" |
| Brand-level catalog description | ✅ Yes | "100+ Indian brands curated for you" |
| Category navigation labels | ✅ Yes | "Indian Fashion" / "Indian Baby" |
| User possessives / personal layer | ❌ Never | "your favourite Mela finds" / "your collection" |
| Persona-targeted campaigns | ⚠️ Sparingly | Only when Indian-American identity IS the explicit context (Diwali guides, heritage gifting) |
| Cultural texture / sub-labels | ✅ Yes (opt-in) | "meri pasand" as italic subtitle, not primary nav label |

### Applied Example: Saved Items Tooltip

| Version | Assessment |
|---------|------------|
| `"Meri Pasand — your favourite Indian finds"` | ❌ Double problem: Hindi jargon primary + "Indian" labels the user |
| `"Meri Pasand — your favourite Mela finds"` | ⚠️ Better, but "Meri Pasand" is still primary (Hindi barrier for Sarah, Arun) |
| `"Saved — your favourite Mela finds"` | ✅ Functional English primary, brand carries cultural identity |
| `"Save to your collection"` | ✅ Best for universal clarity — cultural texture lives in the icon design, not the label |

---

---

## Cross-Cutting UXR Insight: The Inspiration vs. Fulfillment Divide

> *Added: June 2026 — based on first-gen user feedback: "Customers come to eBay/Amazon knowing what they want. On Mela they would come for inspiration."*
>
> *Source: Wife of founder — came to US via marriage, shops actively from Indian brands*

### The Core Behavioral Shift

Users arrive on platforms in fundamentally different shopping modes. Mela sits in a distinct position that most existing Indian product channels do not occupy:

| Platform | Shopping Mode | User State | UX Optimized For |
|----------|--------------|-----------|-----------------|
| Amazon / eBay | **Fulfillment** | "I know what I want. Help me find it fast." | Search, price comparison, quick checkout |
| Myntra / FirstCry | **Fulfillment** | "I know the brand/product. Show me inventory." | Brand navigation, size/filter, fast checkout |
| Pinterest / Instagram | **Inspiration** | "Show me something I'll love." | Browse, discovery, visual serendipity |
| **Mela** | **Inspiration → Fulfillment** | "I came to discover. I leave knowing what I want." | Curated browse + trust signals + confident checkout |

### Why This Changes the UX Priority Order

This insight inverts the common assumption that search is the most important UX surface. For Mela:

1. **Discovery surfaces come first**: Homepage, curated collections, "shop by occasion," editorial content — these are where Mela wins. A customer who arrives with loose intent ("something Indian for the nursery") and leaves having discovered a brand they now trust has had a successful Mela session.

2. **Search is the fallback, not the default**: Neha may use search to find a specific brand she already knows. But she came to Mela because Mela can also show her things Myntra wouldn't surface for a US shopper. Both paths matter, but the investment priority should be discovery-first.

3. **Curation is the core product**: The fact that users come for inspiration means Mela's editorial judgment — what brands it carries, how it groups products, what stories it tells — is itself the value proposition. Mela is not a search engine pointing at Indian inventory. It is a curated discovery experience that happens to have checkout.

### Per-Persona Mode Analysis

| Persona | Arrives in Mode | Mela's Unique Role |
|---------|----------------|-------------------|
| Sarah | Pure inspiration — "something sustainable I can't find on Etsy" | Introduce her to an artisan universe she couldn't discover elsewhere |
| Priya | Occasion-led — "something for Diwali but not sure what" | Curate by occasion so she discovers the right item before she knew to search for it |
| Neha | Split — fulfillment (specific brand) + inspiration (new discoveries) | Surface known brands fast; use recommendations to expand her repertoire |
| Arun | Pure inspiration — has no frame of reference for what to want | Cultural context + visual storytelling to make the first discovery feel safe |

### The Design Principle

> **Mela is a destination for discovery, not a shortcut to India's Amazon.**

A user who could go to Myntra to find what they want chooses Mela because Mela shows them what they didn't know they wanted. That is a fundamentally different value proposition, and the product should be built accordingly: discovery-first, curation-led, with checkout as the conclusion of an inspirational journey — not the purpose of a search session.

---

## Persona Updates

| Date | Update |
|------|--------|
| Jan 2026 | Initial personas created based on market research |
| Apr 2026 | Added per-persona "Relationship With 'Indian' in Copy" sections; added Cross-Cutting UXR Insight: The "Indian" Anchoring Principle |
| Jun 2026 | Updated Neha's shopping behavior to reflect dual-mode (fulfillment on Myntra, inspiration on Mela); added "Shopping Mode on Mela" subsection with "came via marriage" sub-variant; added "Shopping mode on Mela" row to comparison matrix; added Cross-Cutting UXR Insight: The Inspiration vs. Fulfillment Divide (source: first-gen user feedback) |

---

*Last updated: June 2026*
*Source: Mela market research, why-mela-matters analysis, copy review of saved-items feature, first-gen user feedback (founder's wife)*
