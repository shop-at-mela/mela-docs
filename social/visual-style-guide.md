...
1. **Source still — default to a Blotato lifestyle scene** for the product (Product Scene Placement — a staged warm scene beats a plain white/grey catalog shot and matches the Mela aesthetic). **Two exceptions → real photo:** (a) **people/faces/garment-drape on a person** (kidswear on a child) — the hard line, diffusion warps faces + drape; (b) a **strong lifestyle gallery shot already exists** — source-select it, don't regenerate. Full rule: `visual-scorecard.md` Stage 0.
2. **Morph gate for detailed products** (fine gold/hand-paint, jewelry, intricate motif, label-forward): still Blotato-stage, but with a **tight literal prompt** ("keep the product + its exact motif unchanged, only build the environment") + **diff-vs-source QC** + **show the founder the render before shipping** (never silently ship a detailed-piece render — the founder makes the fidelity-vs-scene call; ref Kaunteya 2026-09-05). Rigid/simple products (footwear, homeware, bottles): quick QC only. Same gate applies to Pinterest pins — morph happens at generation, not display.
